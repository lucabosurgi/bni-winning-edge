# Winning Edge Partners — member directory + AI concierge

A standalone website for the Winning Edge Partners network (Spring Hill, FL):
...
- **AI concierge** — a chat widget that routes visitors to the right member and
  **captures each request as a lead/referral**, attributed to Luca for Winning Edge stats.
...
3. Add the same environment variables from `.env.local` in Vercel's project
   settings.
4. Deploy. Add your custom domain (e.g. `winningedgepartners.com`) under
   **Settings → Domains**.
...
your referral reporting — no need to open Supabase. The page is private
...
- **Neutral branding** on purpose — it's the network's asset, so every member
